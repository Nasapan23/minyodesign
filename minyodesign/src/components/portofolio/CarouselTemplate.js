...
import { Loader } from '@react-three/drei'; // Added Loader for suspense fallback
...
export default function Carousel({
  ...
  // Memoize handler functions to avoid unnecessary re-renders
  ...
  // Memoize positions and scales to avoid recalculation on every render
  ...
  return (
    <div {...handlers} className={`flex justify-center items-center ${isMobile ? 'h-[50vh]' : 'h-screen'} w-full bg-black pb-8`}>
      <div className="w-full h-full">
          <Canvas camera={{ position: [0, 0, 10], fov: isMobile ? 45 : 45 }}> {/* Single Canvas */}
            ...
            <InteractiveDisketa initialPosition={[-1, isMobile ? -0.2 : -1.5, 0]} floatSpeed={0.5} amplitude={0.2} phaseOffset={0}>
